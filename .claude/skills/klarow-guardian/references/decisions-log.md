...
|---|---|---|---|---|
| **Krój Geist zamiast Nunito Sans; tekst na `/` cięty o 86 %; osią strony jest dowód, nie opis** | 2026-09-17 | Karol | „Załadujmy 90% mniej tekstu. (...) strona jest bardziej prestiżowa, ale nie wygląda premium. (...) scrolując próbujmy coś udowodnić. Nie popisujmy się aż tak narzędziami". Research rynkowy postawił kierunek: MALINSKI.AI gra objętością (4500–5000 słów, 11 sekcji, 32 opisane narzędzia), basement.studio przeciwnie (450–500 słów, cztery prace). Z bliźniakiem nie wygramy na jego polu, bo ma rok przewagi i więcej do opisania, więc gramy na przeciwnym. Typografia jest w badaniach czynnikiem numer jeden odbioru „premium", a Nunito Sans jest krojem humanistycznym: mówi „przyjazny" tam, gdzie ma mówić „precyzyjny". Krój WYMIENIONY, nie dodany, więc budżet zszedł o 47 % | `design-typography-scale` (Geist solo, tracking nagłówków `-.015em`), `perf-fonts-budget` (49,9 KB z limitu 100 KB), `copy-minimal-text` (bez zmian w treści reguły; `/` zeszła z 1272 do 182 słów), `seo-prerender-must-keep` (shell wypisuje TE SAME zdania co klient — gęsty shell przy rzadkiej stronie to cloaking, nie optymalizacja; długi ogon przeniesiony na podstrony, które już go mają) |
