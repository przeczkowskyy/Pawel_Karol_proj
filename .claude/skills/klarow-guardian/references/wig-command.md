# Web Interface Guidelines (Vercel) — kopia lokalna dla strażnika

> **Źródło:** `https://raw.githubusercontent.com/vercel-labs/web-interface-guidelines/main/command.md`
> (skill `web-design-guidelines` z `vercel-labs/agent-skills` trzyma tylko `SKILL.md` i pobiera te reguły zdalnie).
> **Data pobrania:** 2026-09-11 (190 linii, 7 760 B). **Licencja:** MIT, © Vercel (`vercel-labs/agent-skills`).
...
>
> Precedencja: `klarow-guardian` (`rules/*.md`) > CLAUDE.md > `motion` > `design-taste-frontend` > **ten dokument**.
> Gdzie reguła strażnika mówi inaczej (Title Case w nagłówkach, `&` zamiast „i", em-dash, `autocomplete="off"`),
> obowiązuje strażnik: patrz `references/audit-checklist.md` §4 i reguły `i18n-*`, `copy-*`.
> Mapowanie WIG → reguły strażnika: `references/audit-checklist.md` §5.
