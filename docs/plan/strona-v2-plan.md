...
5. Motion = `motion@13.2.0` w wariancie oszczędnym (`LazyMotion domAnimation` + `m.*` + `MotionConfig reducedMotion="user"`, ≤ 36 KB gz), **3 komponenty ruchu** (`Reveal`/`RevealGroup`, `PageFade`, `ChartReveal`) plus warstwa mediów (`HeroMedia`, `MediaBoundary`, `ChaosToOrder`; §6.3), 4 czasy, 1 krzywa, zero scroll-linked, zero pinowania; React 19.3 z `<ViewTransition>` dopiero w fazie 2.
6. **Wideo w v1: dokładnie jedno autoodtwarzane na `/`, i jest nim NAGRANIE PRAWDZIWEGO NARZĘDZIA** (D36, D37, korekta wieczorna 2026-09-12; wcześniej brzmiało „zero wideo w v1", D28/D13). Kadr prawdziwego pulpitu produkcji (WebP ≤ 110 KB desktop, ≤ 55 KB mobile, `fetchpriority="high"`) **zostaje i nadal jest LCP**, bo jest klatką zero ośmiosekundowego nagrania tego samego pulpitu, które startuje po `window.load` wyłącznie na desktopie; ściana S3 dostaje cztery klipy hover. GLSL Hills schodzi z `/` w F0 i nie wraca. Higgsfield kupujemy wąsko: **wyłącznie statyczna faktura** (grunt hero, tło OG ≈ 56 kredytów, czyli 0 USD w trialu) plus opcjonalna pętla na LinkedIn poza stroną (49 USD). Telefon nadal nie dostaje ani jednego bajtu wideo (§7.1, `warstwa-wrazenia.md`).
...
9. Realna estymata po korekcie wieczornej: **16–17 dni roboczych** (F0 4 + F1 2 + F2 4 + F3 3 + F4 2 + F5 1–2; arytmetyka w §11) i ~4 tygodnie kalendarzowe. F1 urosło do 2 dni (nagrania narzędzi, potok klatkowy, bramka czytelności, opcjonalny trial Higgsfielda) i **stoi na ścieżce krytycznej**: zrzuty i nagrania narzędzi wymagają `DashboardMount`, zdjętego `zoom` roota i przyciętego `company-ui.css`, czyli DoD fazy F0, a bez nich hero i ściana w F2 stoją na placeholderach. Ścieżka krytyczna: decyzje founderów, **tożsamość administratora do `/rodo`**, portrety i bio, zrzuty i nagranie hero (F1; blokujące dla F2 jest wyłącznie nagranie N1) oraz umowa IP z poprzednią firmą.
