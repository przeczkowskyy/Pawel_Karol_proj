...
/* GLSLHills (port 21st.dev) — animowane wzgórza z szumu Perlina (RawShader),
   ...
   - throttle ~30 fps (tło nie potrzebuje 60). */
...
void main(void) {
  ...
  /* stal Klarow #A8B4C2 zamiast neutralnej szarości oryginału */
  vec3 color = vec3(0.659, 0.706, 0.761);
  ...
  useEffect(() => {
    ...
    const onVisibility = () => {
      if (document.hidden) {
        ...
        clock.getDelta(); /* zrzuć zaległy delta-time */
      } else if (raf === null && !reduce) {
        raf = requestAnimationFrame(loop);
      }
    };

    if (reduce) {
      uniforms.time.value = 8; /* jedna statyczna, "rozwinięta" klatka */
      renderer.render(scene, camera);
    } else {
      raf = requestAnimationFrame(loop);
